...
import { executeBotKnowledgeUpdate, botKnowledgeUpdateSchema } from './tools/bot-knowledge-update.js';
...
function apiKeyAuth(req: AuthRequest, res: Response, next: NextFunction): Response | void {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    logger.warn({ requestId: req.requestId, path: req.path }, 'Missing or invalid authorization header');
  ...
  logger.info({ requestId: req.requestId, apiKeyHint: req.apiKeyHint, path: req.path }, 'Request authenticated');
  next();
}
