...
export async function executeKnowledgeIngest(input: KnowledgeIngestInput): Promise<string> {
  ...
  const title = lines.length > 0 ? lines[0].substring(0, 100) : content.substring(0, 100);
  ...
  try {
    ...
    throw new Error(
      `Knowledge ingestion failed: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export const knowledgeIngestTool = {
  ...
  description:
    'Ingest and process knowledge from multiple sources for daily updates. Supports RSS, APIs, documents, and manual input.',
  inputSchema: {
