...
const configSchema = z.object({
  ...
  // API Security
  MCP_API_KEYS: z.string().default('').transform(val => 
    val ? val.split(',').map(k => k.trim()).filter(Boolean) : []
  ),
  
  // Web Fetch Security
  ALLOWED_DOMAINS: z.string().default('wikipedia.org,github.com,.edu,.gov').transform(val =>
